...
#'
#' @examples
plot4server_QSBS_ave  <- function(gof, r.period, min_years, max_years, stations2ave = stations2average(min_years, max_years)) { 
    ...
    for (m in 1:3) {  # TO FIX for Bayes
      ...
      for (st in seq(along = stations2ave)) {
        ...
        if (gof == "NT") {
          ...
          # coefficient of variation = st_dev / mean
          temp_top <- array(NA, dim = c(length(sampling_years), 6))  # We load QS or BS for all reutrn periods before averaging the results
          temp_top <- var.get.nc(gof_nc, gof, start = c(stations2ave[st], d, m, 1, 2, 1), 
                     count = c(1, 1, 1, length(sampling_years), 1, 6))
          
          temp_bot <- array(NA, dim = c(length(sampling_years), 6))  # We load QS or BS for all reutrn periods before averaging the results
          temp_bot <- var.get.nc(gof_nc, gof, start = c(stations2ave[st], d, m, 1, 1, 1), 
                       count = c(1, 1, 1, length(sampling_years), 1, 6))
          
          temp2plot_raw[st, m, ] <- rowMeans(temp_top, na.rm = TRUE) / rowMeans(temp_bot, na.rm = TRUE)
    ...
    for (d in seq(along = distr.name)) {
      ...
      for (st in seq(along = stations2ave)) {
        ...
        if (gof == "NT") {
          ...
          temp2plot_raw[st, d, ] <- rowMeans(temp_top, na.rm = TRUE) / rowMeans(temp_bot, na.rm = TRUE)
      ...
      temp2plot[d, ] <- colMeans(temp2plot_raw[, d, ], na.rm = TRUE)  # Averaging over all stations
    }
